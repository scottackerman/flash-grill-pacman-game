package {
	...
	public class GrillMan_Nationals_300x250 extends MovieClip {
		private var grillFire:Flamer;
		private var bgFlame:Flamer;
		private var grillScale:Number = .20;
		...
		private var grillMovementIncrement:uint = 65;
		private var halfGrillWidth:Number = 32;
		...
		private var MEAT_MAX_DROP_SPEED_ARRAY:Array = new Array(2, 2.3, 1.2, 1.6, 1);
		private var MAX_DROP_SPEED:Number = 2;
		private var MIN_DROP_SPEED:Number = .5;
		...
		public function GrillMan_Nationals_300x250() {
			stageWidth = stage.stageWidth;
			stageHeight = stage.stageHeight;
			grillFire = new Flamer(grill_fire_mc);
			addChild(grillFire);
			bgFlame = new Flamer(paperCover_mc);
			addChild(bgFlame);
			...
			play_again_btn.x = 237;
			play_again_btn.y = 198;
			play_again_btn.scaleX = play_again_btn.scaleY = .85;
		...
		private function gameOver():void {
			...
			TweenLite.to(grill_fire_mc, .5, {x:500, ease:Quad.easeOut});
			TweenLite.to(grill_mc, .5, {x:500, ease:Quad.easeOut});
		...
		function fireOut():void {
			TweenLite.to(paperCover_mc, 1, {scaleX:2, scaleY:2, alpha:0, ease:Quad.easeIn});
			grill_mc.scaleX = -grillScale;
			grill_fire_mc.scaleX = -grillScale;
			grill_fire_mc.vectorFlames_mc.alpha = 1;
			TweenLite.to(grill_fire_mc, .5, {x:40, ease:Quad.easeOut, delay:.5});
			TweenLite.to(grill_mc, .5, {x:40, ease:Quad.easeOut, delay:.5, onComplete:resetGrill});
		}
		...
		private function resetGrill():void {
			...
			TweenLite.to(grill_fire_mc, .5, {x:145, ease:Quad.easeOut});
			TweenLite.to(grill_mc, .5, {x:145, ease:Quad.easeOut, onComplete:showPlayAgainButton});
		}
		...
		function hitGrill(evt:Event):void {
			if (evt.target.hitTestObject(grill_fire_mc.body_mc.bottom_teeth_mc)) {
		...
		private function legalRollOver(evt:Event):void {
			TweenLite.to(evt.target, .2, {y:215, ease:Quad.easeOut});
		}
		
		private function legalRollOut(evt:Event):void {
			TweenLite.to(evt.target, .2, {y:235, ease:Quad.easeOut});
		}
