package {
	...
	public class GrillMan_160x600 extends MovieClip {
		private var grillFire:Flamer;
		private var bgFlame:Flamer;
		private var grillScale:Number = .175;
		...
		private var grillMovementIncrement:uint = 30;
		...
		public function GrillMan_160x600() {
			stageWidth = stage.stageWidth;
			stageHeight = stage.stageHeight;
			grillFire = new Flamer(grill_fire_mc, 600, 600, null, true);
			addChild(grillFire);
			bgFlame = new Flamer(paperCover_mc, 300, 750, null, true);
			addChild(bgFlame);
			...
			play_again_btn.x = stageWidth/2;
			play_again_btn.y = 400;
			play_again_btn.scaleX = play_again_btn.scaleY = 1.2;
		...
		private function resetGrill():void {
			...
			TweenLite.to(grill_fire_mc, .5, {x:128, ease:Quad.easeOut});
			TweenLite.to(grill_mc, .5, {x:128, ease:Quad.easeOut, onComplete:showPlayAgainButton});
		}
		...
		private function legalRollOver(evt:Event):void {
			TweenLite.to(evt.target, .2, {y:555, ease:Quad.easeOut});
		}
		
		private function legalRollOut(evt:Event):void {
			TweenLite.to(evt.target, .2, {y:585, ease:Quad.easeOut});
		}
